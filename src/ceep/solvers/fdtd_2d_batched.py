...
class BatchedFDTD2D:
    ...
    def _apply_cpml(self):
        """
        Apply Convolutional PML (CPML) absorbing boundary corrections.

        Implementation based on Roden & Gedney (2000) and Taflove & Hagness.
        This is called after each field update to apply PML damping.
        """
        cp = self.xp
        n = self.cpml_n
        nx, ny = self.nx, self.ny
        ...
        # ====================================================================
        # CPML for H-field (already updated)
        # ====================================================================

        # Left boundary (x = 0 to n-1)
        for i in range(n):
            # dEz/dy for Hx update in PML
        ...
        # ====================================================================
        # CPML for E-field (already updated)
        # ====================================================================

        # Left boundary (x = 0 to n-1) - Ez corrections from dHy/dx
        for i in range(n):
            if i+1 < nx:
        ...
        for i in range(n):
            x_idx = nx - n + i
            if x_idx > 0:
                curl_H_x = (self.hy[:, x_idx, :] - self.hy[:, x_idx-1, :]) * self.inv_dx
        ...
        # Bottom boundary (y = 0 to n-1) - Ez corrections from -dHx/dy
        for j in range(n):
            if j+1 < ny:
        ...
        for j in range(n):
            y_idx = ny - n + j
            if y_idx > 0:
                curl_H_y = -(self.hx[:, :, y_idx] - self.hx[:, :, y_idx-1]) * self.inv_dy
    ...
    def run(self) -> Dict[int, Dict[int, np.ndarray]]:
        ...
        for step in range(self.total_steps):
            ...
            # CPML absorbing boundaries (Roden & Gedney 2000 formulation)
            self._apply_cpml()
