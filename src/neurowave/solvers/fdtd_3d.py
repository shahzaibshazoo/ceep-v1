...
from neurowave.core.config import SimulationConfig
from neurowave.core.grid_3d import Grid3D
from neurowave.core.base import BaseSource
from neurowave.core.backend import to_scalar, to_numpy, get_backend_module, is_gpu_active
...
class FDTD3D:
    ...
    def _update_h_fields(self) -> None:
        ...
        if is_gpu_active() and self._use_fused_kernels:
            from neurowave.cuda.kernels import launch_update_h_3d
    ...
    def _update_e_fields(self) -> None:
        ...
        if is_gpu_active() and self._use_fused_kernels:
            from neurowave.cuda.kernels import launch_update_e_3d
            nx, ny, nz = self.grid.nx, self.grid.ny, self.grid.nz
            launch_update_e_3d(
                self.grid.ex, self.grid.ey, self.grid.ez,
                self.grid.hx, self.grid.hy, self.grid.hz,
